...
export default async (argv: { [key: string]: string | string[] } | DownloadOptions) => {
  if (!shell.which("git")) {
  ...
  if ('config' in argv || 'git-repo' in argv) {
    ...
    configJson = argv as DownloadOptions;
  }

  if (!configJson.repos || configJson.repos.length === 0) {
    shell.echo(
      "missing `repos` configuration, you can set through `--git-repo` argument or set repos field in config file.\n"
    );
    return shell.exit(1);
  }
