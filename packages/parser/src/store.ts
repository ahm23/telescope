...
export class ProtoStore implements IProtoStore {
  ...
  processProtos(
    ...
    return contents.map(({ absolute, filename, content }) => {
      try {
        ...
        if (
          this.options.prototypes.patch &&
          this.options.prototypes.patch[filename]
        ) {
            ...
            const result = applyPatch(
              ...
              ops.map((op) => {
                if (op.path.startsWith("@")) {
                  op.path =
                    convertPackageNameToNestedJSONPath(protoJson.package) +
                    op.path.substring(1);
                }
                return op;
              })
  ...
  traverseAll(): void {
    ...
    this.protos = this.getProtos()
      .map((ref: ProtoRef) => {
        const isHardExcluded =
          this.options?.prototypes?.excluded?.hardProtos &&
          isRefExcluded(ref, {
        ...
        if (!actualFiles.has(ref.filename)) {
          ...
          const isIncluded = isRefIncluded(
            ref,
            this.options.prototypes.includes
          );
          const isExcluded = isRefExcluded(
            ref,
            this.options.prototypes.excluded
          );
