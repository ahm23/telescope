...
async function main() {
    ...
    server.prompt('codegen-usage', 'Guide for using telescope generated code', async () => {
    ...
    server.prompt('agent-guidelines', 'Guidelines for MCP agents using outputicjs', async () => {
    ...
    server.tool('create-blockchain-function', 'Create custom blockchain functions by referencing telescope examples and generated code', {
        ...
        try {
            ...
            return {
                content: [{
                        type: "text",
                        text: `Error generating blockchain function guidance: ${error.message}`
                    }]
            };
        }
    });
...
function getRelevantExamples(task, availableExamples) {
    ...
    return [...new Set(relevantExamples)].filter(example => availableExamples.includes(example));
}
